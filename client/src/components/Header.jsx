  ...
  return (
    <Navbar className='border-b-2'>
      ...
      <div className="flex gap-2 md:order-2">
        <Button className='w-12 hidden sm:inline h-10' color='gray' pill onClick={()=>{dispatch(toggleTheme())}}>
          {theme==='light'?<FaSun/>:<FaMoon/>}
          
        </Button>
        {currentUser ? (
          <Dropdown arrowIcon={false} inline label={
            <Avatar alt='user' img={currentUser.profilePicture} rounded className='object-contain'/>
          }>
            <Dropdown.Header>
        ...
        (
          <Link to='/sign-in'>
          <Button gradientDuoTone='purpleToBlue' pill outline>Sign In
